...
const type = [
  ...
  "toAirport",
  "fromAirportAndCity",
  "fromAirportAndToAirport",
  "cityAndToAirport",
  "fromAirportAndCityAndToAirport"
];
module.exports = function(Trip) {
  Trip.addPayment = function(
    tripId,
    price,
    cardHolderName,
    cardNumber,
  ...
  Trip.beforeRemote("create", function(context, result, next) {
    ...
    cheackLocation(data.locationId, function(err) {
      ...
      cheackCar(data.carId, data.type, bookingDate.where, null, function(err) {
        ...
        if (data["couponId"] == undefined) next();
        else {
          Trip.app.models.coupon.find(
            ...
            function(err, oneCoupon) {
              ...
              if (oneCoupon[0].type == "fixed")
                ...
                newCost =
                  context.req.body.costBeforCoupon -
                  (context.req.body.costBeforCoupon * oneCoupon[0].value) / 100;
              context.req.body.cost = newCost;
              context.req.body.travelAgencyId = oneCoupon[0].travelAgencyId;
              Trip.app.models.travelAgency.findById(
                context.req.body.travelAgencyId,
                function(err, travelAgencyData) {
                  if (err) return next();
                  if (travelAgencyData == null) return next();
                  context.req.body.travelAgencyDiscountType =
                    travelAgencyData.type;
                  context.req.body.travelAgencyDiscountValue =
                    travelAgencyData.value;
                  next();
                }
              );
            }
  ...
  Trip.afterRemote("create", function(ctx, result, next) {
    ...
    Trip.app.models.bookingCar.create(bookingData, function(err, data) {
      ...
      if (ctx.req.body.tripSublocations)
        Trip.app.models.tripSublocation.create(
          ctx.req.body.tripSublocations,
          function(err, data) {
            if (err) return next(err);
            if (ctx.req.tempBody) {
              addPaymentTrip(
  ...
  async function generatePdf(trip) {
    var path = require("path");
      ...
      var renderer = loopback.template(
        path.resolve(__dirname, "../../templates/trip/index.html")
      );
      var car = trip.car();
    ...
    return new Promise((resolve, reject) => {
      ...
      .toFile(__dirname + "../../../../madarImage/uploadFiles/pdf/" + file, (error, res) => resolve({ error: error, res: file }));
    });
  }
  ...
  function cheackCar(carId, type, where, tripId, callback) {
    Trip.app.models.car.findById(carId, function(err, oneCar) {
      ...
      Trip.app.models.bookingCar.find(mainWher, function(err, data) {
        if (err) return callback(err);
        if (data[0] != null) return callback(errors.car.carNotAvailable());
        callback(null);
      });
    });
  }
  ...
  Trip.changeStatus = function(id, newStatus, callback) {
    ...
    Trip.find(
      ...
      function(err, trip) {
        if (err) return callback(err);
        if (trip[0] == null) return callback(errors.trip.tripNotFound());
        ...
        if (newStatus != "finished") {
          ...
          Trip.app.models.userNotification.sendRateNotification(
            trip[0].ownerId,
            trip[0].id,
            trip[0].carId,
            function(err) {
    ...
    if (filter != {} && filter != undefined) {
      for (let index = 0; index < filter["$and"].length; index++) {
        ...
        if (element["endDate"] != null) {
          filter["$and"][index]["endDate"]["$gt"] = new Date(
            element["endDate"]["$gt"]
          );
        }
        if (element["startDate"] != null) {
          filter["$and"][index]["startDate"]["$lt"] = new Date(
            element["startDate"]["$lt"]
          );
        }
      }
      where = [
  ...
  Trip.editMethod = function(data, id, context, callback) {
    ...
    Trip.findById(id, function(err, TripData) {
      ...
      if (data.ownerId == null) data.ownerId = context.req.accessToken.userId;
          ...
          deleteCarBooking(id, function(err) {
            ...
            deleteTripSubLocation(id, function(err) {
              ...
              TripData.updateAttributes(newData, function(err, newTripData) {
                ...
                Trip.app.models.bookingCar.create(bookingData, function(
                  err,
                  data
                ) {
                  if (err) return callback(err);
    ...
    Trip.findById(tripId, function(err, trip) {
      ...
      if (mainCouponId == couponId) {
        ...
        Trip.app.models.coupon.find(
          ...
          function(err, oneCoupon) {
            ...
            if (oneCoupon[0].type == "fixed")
              ...
              newCost =
                trip.costBeforCoupon -
                (trip.costBeforCoupon * oneCoupon[0].value) / 100;
        ...
        Trip.app.models.coupon.findById(mainCouponId, function(err, coupon) {
          if (err) return callback(err);
          coupon.numberOfUsed--;
          if (coupon.status == "used") coupon.status = "active";
          coupon.save();
